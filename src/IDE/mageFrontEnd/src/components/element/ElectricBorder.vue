...
const updateAnim = () => {
    ...
    const baseDur = 6;
    const dur = Math.max(0.001, baseDur / (0));
    [...dyAnims, ...dxAnims].forEach(a => a.setAttribute('dur', `${dur}s`));

    const disp = svg.querySelector('feDisplacementMap');
    if (disp) disp.setAttribute('scale', String(30 * (props.chaos || 0)));
...
<template>
    <div ref="rootRef" :class="['relative isolate', className]" :style="style">
        ...
        <div class="absolute inset-0 pointer-events-none" :style="inheritRadius">
            ...
            <div class="box-border absolute inset-0" :style="glow1Style" style="height: 400px;" />
            <div class="box-border absolute inset-0" :style="glow2Style" style="height: 400px;" />
            <div class="absolute inset-0" :style="bgGlowStyle" style="height: 400px;" />
        </div>
