...
const baseClassName = computed(() => {
    return `card flex flex-col justify-between relative aspect-[4/3] min-h-[200px] w-full max-w-full p-5 rounded-[20px] border border-solid font-light overflow-hidden transition-[box-shadow] duration-300 ease-in-out hover:shadow-[0_8px_25px_rgba(0,0,0,0.15)] ${props.enableBorderGlow ? 'card--border-glow' : ''
        }`;
});
...
<template>
    <GlobalSpotlight v-if="enableSpotlight" :grid-ref="gridRef" :disable-animations="shouldDisableAnimations"
        :enabled="enableSpotlight" :spotlight-radius="spotlightRadius" :glow-color="glowColor" />

    <BentoCardGrid :grid-ref="(el: HTMLDivElement | null) => {
        gridRef = el;
    }
        ">
        <div class="gap-2 grid card-responsive">
            <template v-for="(card, index) in cardData" :key="index">
                <ParticleCard v-if="enableStars" :class="baseClassName" :style="getCardStyle(card)"
                    :disable-animations="shouldDisableAnimations" :particle-count="particleCount"
                    :glow-color="glowColor" :enable-tilt="enableTilt" :click-effect="clickEffect"
                ...
                <div v-else :class="baseClassName" :style="getCardStyle(card)"
