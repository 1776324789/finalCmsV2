...
const FrontServer = () => {
    ...
    function loadWebsiteConfig() {
        try {
            const dbPath = path.join('IDE', 'server', 'DataBase', 'systemData.json')
            if (fs.existsSync(dbPath)) {
                const db = JSON.parse(fs.readFileSync(dbPath, 'utf-8'))
                if (db.website && Array.isArray(db.website)) {
                    db.website.forEach(site => {
                        websiteConfigMap[site.target] = {
                            status: site.status !== false, // 默认为 true
                            id: site.id
                        }
                    })
                }
    ...
    function isSiteEnabled(siteName) {
        // 如果没有配置，默认启用
        const config = websiteConfigMap[siteName]
        return config ? config.status : true
    }
    ...
    function resolvePagePath(urlPath, webDirs) {
        ...
        // ✅ 检查站点状态，如果停用则返回 403 页面
        if (!isSiteEnabled(site)) {
            return path.join('Front', 'ROOT', '403.html')
        }
