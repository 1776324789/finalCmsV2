...
function loadBaseData() {
    ...
    if (local) {
        const checkModify = () => {
            fetch(BaseCmsURL + `/ModifyDate.json?data=` + Date.now())
...
function dp(pageString) {
    ...
    return {
        element: content.childNodes[0], children: domElement
    }
}
...
class NodePagination extends CmsElement {
    ...
    getValue() {
        return {
            total: this.totalTem, page: this.pageTem, limit: this.limitTem
        }
    }
    ...
    generatePaginationIndexes() {
        ...
        if (pageIndexes[pageIndexes.length - 1] < Math.ceil(this.totalTem / this.limitTem)) pageIndexes.push(Math.ceil(this.totalTem / this.limitTem))
        return pageIndexes;
    }
}
...
class ListNode extends CmsElement {
    ...
    loadNodes() {
        ...
        for (let a = start; a < end; a++) {
            ...
            if (this.getAttribute("top") != null) cmsNodeEmlement.setAttribute("top", true)
            cmsNodeEmlement.setAttribute("data", ids[a].id)
            this.appendChild(cmsNodeEmlement)
        }
        if (pagination != null) {
            ...
            if (!this.initPagination) {
                ...
                pagination.onchange = (e) => {
                    if (this.getAttribute("newpage") != null) {
                        if (this.data.template != null && this.data.template != "") toPage("/" + this.data.template + ".html?dataId=" + this.data.id + "&page=" + e.page)
                    } else {
...
console.log(this.data)
               ...
               if(this.data.link!=null&&this.data.link!="")return toPage( this.data.link)
