...
new class Confirm {
    ...
    show(text, yes, no) {
        ...
        //根据窗口大小调整组件所在位置
        let left = window.innerWidth - (this.element.element.offsetLeft + this.element.element.clientWidth)
        left = left < 0 ? left : 0
        this.element.element.style.marginLeft = left - 15 + "px"
    }
}
...
class ContentBlock extends HTMLElement {
    ...
    initElement() {
        ...
        Array.from(this.getElementsByTagName("content-menu")).forEach(item => {
            if (item.parentNode != this) return
    ...
    openContent() {
        ...
        //在卡片展开后将卡片调整到可以进行自适应
        setTimeout(() => {
            if (this.height == this.refs.content.clientHeight) {
    ...
    initContent() {
        this.parentNode.insertBefore(this.element.element, this)
        this.refs.content.appendChild(this)
    }
...
class DialogEngine extends HTMLElement {
    ...
    initElement() {
        ...
        if (this.dialogInner.clientHeight < this.dialogInner.scrollHeight) {
    ...
    initFunctions() {
        this.dialogInner.addEventListener("scroll", (e) => {
            ...
            if (this.foot) {
                if (this.dialogInner.clientHeight + this.dialogInner.scrollTop < this.dialogInner.scrollHeight) {
                    this.foot.style.boxShadow = "0 -2px 2px #dadada"
                } else {
...
class FormItem extends HTMLElement {
    ...
    verify() {
        ...
        if (this.requireTemp) {
            ...
            let verify = this.verifyFun(value)
            //验证结果，通过返回true
            if (!verify) {
    ...
    verifyFun(value) {
        ...
        //为0的情况不算为空
        if (value + "" == "0") value = "0"
        console.log(value == null || value == "" || value == {} || value == [])
    ...
    set labelWidth(value) {
        console.log("set label width : " + value)
        this.refs.label.style.width = value
    }
    //获取表单项中组件的值
    get value() {
        return { name: this.getAttribute("name"), value: this.refs.component.children[0].value }
    }
    //设置表单项组件中的值
    set value(value) {
...
class Form extends HTMLElement {
    ...
    initElement() {
        ...
        if (this.getAttribute("direction") != "column") {
            ...
            Array.from(this.componentMap).forEach(temp => {
                ...
                console.log(item.labelWidth > this.maxLabelWidth, item.labelWidth, this.maxLabelWidth)
    ...
    //提交方法，传入方法后返回表单的值
    submit(fun) {
        if (this.fullValue == null) this.fullValue = {}
        let verify = true
        Array.from(this.componentMap).forEach(item => {
            //验证
            if (item[1].verify()) {
                this.fullValue[item[1].value.name] = item[1].value.value
            } else {
                verify = false
            }
        })
...
class Input extends HTMLElement {
    ...
    initElements() {
        ...
        if (!this.limit)
            this.refs.input.value = this.getAttribute("value") != null ? this.getAttribute("value").substring(0, this.limit) : ""
        else
        ...
        this.refs.input.placeholder = this.getAttribute("placeholder") == null ? "" : this.getAttribute("placeholder")
    ...
    initFunctions() {
        this.refs.input.addEventListener("keydown", (e) => {
            if (typeof this.limit == "number" && this.getAttribute("limit") != null) {
                this.showLimit()
                if (e.keyCode != 39 && e.keyCode != 37 && this.refs.input.value.length >= this.limit && e.keyCode != 8)
                    e.preventDefault();
            }
        })
...
class InputNum extends HTMLElement {
    ...
    initFunctions() {
            ...
            if (this.isfocus) {
                if (e.deltaY > 0) {
                    ...
                    for (let a = 0; a < 9; a++) {
                        ...
                        this.refs['button' + a].innerText = this.refs.input.value * 1 + (4 - index) * this.step
                    }
                } else if (e.deltaY < 0) {
                    ...
                    for (let a = 0; a < 9; a++) {
                        let index = this.refs['button' + a].getAttribute("nowindex") * 1
                        this.refs['button' + a].classList.remove("a-button-" + index)
                        this.refs['button' + a].classList.add("a-button-" + (index + 1 == 9 ? 0 : index + 1))
        ...
        this.refs.input.addEventListener("keydown", (e) => {
            ...
            if (!(code == 39 || code == 37 || code == 8 || (code == 110 || (code >= 96 && code <= 105)) || (code == 190 || (code >= 48 && code <= 57))))
                e.preventDefault();
...
class InputText extends HTMLElement {
    ...
    initElements() {
        ...
        if (this.getAttribute("height") != null && this.getAttribute("height") != "auto") this.refs.input.style.height = "calc(" + this.getAttribute("height") + " - 40px)"
        if (this.getAttribute("placeholder") != null) this.refs.input.placeholder = this.getAttribute("placeholder")
        if (this.getAttribute("width") != null) this.element.element.style.width = this.getAttribute("width")
        if (this.getAttribute("align") != null) this.element.children.input.style.textAlign = this.getAttribute("align")
...
class InputRadio extends HTMLElement {
    ...
    initElement() {
        console.log(Array.from(this.getElementsByTagName("a-radio")))
    ...
    get value() {
        let value = {}
        Array.from(this.getElementsByTagName("a-radio")).forEach(radio => {
            console.log(radio.radioName, radio.checked)
...
class InputDateTime extends HTMLElement {
    ...
    initElement() {
        ...
        if (this.getAttribute("disabled") != null && this.getAttribute("disabled") == "true")
            this.disabled = true
        if (this.getAttribute("placeholder") != null) this.refs.inputText.placeholder = this.getAttribute("placeholder")
        this.appendChild(this.element.element)
...
class Menu extends HTMLElement {
        ...
        if (page.getAttribute("group") == null) {
            ...
            if (this.menuGroupMap.has(page.getAttribute("group"))) {
                ...
                this.menuGroupMap.get(page.getAttribute("group")).element.children.children.appendChild(page
                    .menuButton.element)
                this.menuGroupMap.get(page.getAttribute("group")).element.children.children.style.height = this.menuGroupMap.get(page.getAttribute("group")).element.children.children.children.length * 35 + "px"
            }
            else {
                let menuGroup = {
    ...
    drawMenu() {
        ...
        Array.from(this.menuGroupMap).forEach(item => {
            item = item[1]
            item.element.children.children.style.height = item.element.children.children.children.length * 35 + "px"
        })
        ...
        else this.selectPage(Array.from(this.pageMap)[0][1])
            ...
            groupMenu.element.children.self.onclick = () => {
                if (groupMenu.element.children.children.style.height == "0px") {
                    groupMenu.element.children.children
                        .style.height = groupMenu.element.children.children.children.length * 35 + "px"
                } else {
...
class Pagination extends HTMLElement {
    ...
    initElement() {
        let element = dp(`<div class="a-page-info-block">
			...
			<div class="a-set-block" ref="setting">
				...
				<label>每页展示条数</label>
				<a-select width="75px" style="margin-right:10px;" ref="limitSelect">
				</a-select>
    ...
    initPageLimitArray() {
        ...
        this.refs.limitSelect.option = options
...
class SelectorOption extends HTMLElement {
    constructor() {
        super()
        this.valueData = { label: null, value: null }
        this.disable = false
    ...
    select(value) {
        // 已选择的选项再次点击后无响应
        if (!value && this.selected && !this.ifMutiple) return
        this.selected = value
        this.refs.input.checked = value
        this.onselect({ value: this.valueData, check: this.refs.input.checked, element: this })
    }
    onselect() { }
    multiple() {
    ...
    get selected() { return this.onSelected }
    ...
    set label(value) {
        ...
        this.refs.label.innerHTML = value
    }
...
class Selector extends HTMLElement {
    ...
    initOptionFunctions(option) {
        //添加选项选中的监听事件
        option.onselect = (e) => {
            if (this.valueList.length > 0 && this.valueList[0].element == option) return
            //当为单选时，每次选择都清空选择的值
            if (this.multiple == null) {
                this.valueList.forEach(value => {
                    value.element.selected = false
                })
                ...
                this.valueList.splice(this.valueList.indexOf(e), 1)
        ...
        if (typeof value == "NAN" || typeof value == "string" || typeof value == "number") {
            ...
            try {
                ...
                options.forEach(item => {
                    ...
                    option.select(item.selected == true ? true : false)
...
class ButtonEngine extends HTMLElement {
    ...
    initElement() {
        ...
        // 禁用
        if (this.getAttribute("disabled") != "false" && this.getAttribute("disabled") != null) {
